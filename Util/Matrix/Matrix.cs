...
public abstract class Matrix 
    ...
    public static Vector solveBiConjugateGradient(BorderedDiagonalIdentityMatrix A, Vector b, Vector x, int maxit, double stop_tol, bool useILU)
        {
        ...
        while (i <= maxit)
            ...
            if (rho == 0)
                {
                throw new ExecutionEngineException("BiConjugate Gradient failed to converge - rho is 0");
                }
            ...
            if (p_tilde.dot(q) == 0)
                throw new ExecutionEngineException("P dot Q is 0");
        ...
        if (i > maxit)
            throw new ExecutionEngineException("BiConjugate Gradient failed to converge - max iterations exceeded");
                
        return x;
        }
        
    public static Vector solveBiConjugateGradient(Matrix J, DiagonalMatrix W, DiagonalMatrix M, Vector b, Vector x, int maxit, double stop_tol)
        {
        double b_norm = Math.Sqrt(b.dot(b));
        if (b_norm == 0)
            {
